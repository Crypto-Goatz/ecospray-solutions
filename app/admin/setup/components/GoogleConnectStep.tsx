...
export function GoogleConnectStep({ businessName, onComplete, onBack }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Instructions */}
      <div className="rounded-lg border border-zinc-700 bg-zinc-800 p-4 text-sm text-zinc-300 space-y-2">
        <p className="font-medium text-white">How to get your service account key:</p>
      ...
      {!isDone && (
        <>
          <div
            className="border-2 border-dashed border-zinc-700 rounded-lg p-6 text-center cursor-pointer hover:border-green-500/50 transition-colors"
          ...
          <div className="text-center text-zinc-500 text-xs">or paste the base64-encoded key below</div>

          <textarea
            value={googleKey}
            onChange={(e) => setGoogleKey(e.target.value)}
            className="w-full h-20 rounded-lg border border-zinc-700 bg-zinc-800 px-4 py-2.5 text-white text-xs font-mono placeholder:text-zinc-500 focus:outline-none focus:ring-2 focus:ring-green-500 resize-none"
      ...
      <div className="flex gap-3">
        ...
        {isDone ? (
          ...
          <button
            ...
            className="flex-1 rounded-lg bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 py-3 text-white font-semibold transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
...
function StatusLine({ done, active, label, result }: { done: boolean; active: boolean; label: string; result?: string }) {
  ...
  return (
    <div className="flex items-center gap-2 text-sm">
      ...
      <span className={done ? "text-green-400" : "text-zinc-300"}>{result || label}</span>
    </div>
  );
}
