...
export function ContentGenerationStep({ businessInfo, googleKey, spreadsheetId, onComplete, onBack }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg border border-zinc-700 bg-zinc-800 p-4 text-sm text-zinc-300">
        <p>Get a free Gemini API key from <span className="text-green-400">Google AI Studio</span> (aistudio.google.com).</p>
      </div>
      ...
      <div className="flex gap-3">
        ...
        {isDone ? (
          ...
          <div className="flex-1 flex gap-2">
            <button
              onClick={handleSkip}
              className="flex-1 rounded-lg border border-zinc-700 py-3 text-zinc-400 font-medium hover:bg-zinc-800 transition-colors text-sm"
            >
              Skip
            </button>
            <button
              ...
              className="flex-1 rounded-lg bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 py-3 text-white font-semibold transition-colors disabled:opacity-50"
            >
